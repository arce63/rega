...
client.on("guildMemberAdd", member => {
  member.roles.add('874406293241221210'); // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
});
...
Tagımıza ulaşmak için herhangi bir kanala \`${ayarlar.prefix}tag\` yazman yeterlidir. Şimdiden iyi eğlenceler <@&874406276707258388> ! :tada: :tada:`)
  })

// 
client.on("userUpdate", async (say, yeni) => {
  var sunucu = client.guilds.cache.get('874405505286672504'); 
  var uye = sunucu.members.cache.get(yeni.id);
  var normalTag = config.tagsız;
  var ekipTag = config.taglı; 
  var ekipRolü = config.ekiprol; 
  var logKanali = config.taglog; 
  ...
  if ((yeni.username).includes(ekipTag) && !uye.roles.cache.has(ekipRolü)) {
    try {
      ...
      await client.channels.cache.get(config.taglog).send(new Discord.MessageEmbed().setColor('GREEN').setDescription(`${yeni} adlı üye tagımızı alarak aramıza katıldı!`));
  ...
  if (!(yeni.username).includes(ekipTag) && uye.roles.cache.has(ekipRolü)) {
    try {
      ...
      await client.channels.cache.get(config.taglog).send(new Discord.MessageEmbed().setColor('RED').setDescription(`${yeni} adlı üye tagımızı bırakarak aramızdan ayrıldı!`));
    } catch(err) { console.error(err) };
  };
});
...
client.on("userUpdate", async (stg, yeni) => {
  var sunucu = client.guilds.cache.get('874405505286672504'); // Buraya Sunucu ID
  var uye = sunucu.members.cache.get(yeni.id);
  var tag = "✫"; // Buraya Ekip Tag
  var tagrol = "874406289701240912"; // Buraya Ekip Rolünün ID
  var logKanali = "874406449042833419"; // Loglanacağı Kanalın ID
  ...
  if (!(yeni.username).includes(tag) && uye.roles.cache.has(tagrol)) {
    try {
      ...
      await uye.send(`Tagımızı bıraktığın için family rolün ve yetkili rollerin alındı! Tagımızı tekrar alıp aramıza katılmak istersen;\nTagımız: **${tag}**`);
...
client.on('guildMemberAdd',async member => {
  let kisi = client.users.get(member.id);
    ...
    if (zaman < 14) 
  member.addRole("874406326481084446")
  member.removeRole("874406293241221210")
});
...
client.on("guildMemberAdd", member => {
    ...
    const kytsz = member.guild.roles.cache.find(r => r.id === "874406293241221210") 
     var rol = member.guild.roles.cache.get("874406326481084446") // ŞÜPHELİ HESAP ROLÜNÜN İDSİNİ GİRİN
     var kayıtsız = member.guild.roles.cache.get(kytsz) // UNREGİSTER ROLÜNÜN İDSİNİ GİRİN
     member.roles.add(rol)
     member.roles.remove(kytsz)

  member.user.send('Selam Dostum Ne Yazık ki Sana Kötü Bir Haberim Var Hesabın 1 Hafta Gibi Kısa Bir Sürede Açıldığı İçin Fake Hesap Katagorisine Giriyorsun Lütfen Bir Yetkiliyle İletişime Geç Onlar Sana Yardımcı Olucaktır İyi Günler.')
  setTimeout(() => {
...
client.on("guildMemberAdd", member => {
  let sunucuid = "874405505286672504"; //Buraya sunucunuzun IDsini yazın
  let tag = "✫"; //Buraya tagınızı yazın
  let rol = "874406289701240912"; //Buraya tag alındığı zaman verilecek rolün IDsini yazın
...
member.roles.add(rol)
  const tagalma = new Discord.MessageEmbed()
      ...
      .setTimestamp()
     client.channels.cache.get('874406449042833419').send(tagalma)
}
})
